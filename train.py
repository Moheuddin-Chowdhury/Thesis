#Create a folder named "PP_SAM" into your drive
#Create three folder into "PP_SAM"  
    #dataset(traindata,testdata)
    #Model (vit_b or any other vision transform model)
    #Model data
#upload a pretrained model path (ex : pvt_v2_b2.pth)
...
#Create environment
!conda create -n emcadenv python=3.8 -y
!source activate emcadenv
...
import sys
sys.path.append('/content/drive/MyDrive/PP_SAM')
...
def test(model, path, dataset):
    ...
    for i in range(num1):
        ...
        # add percentage to bounding box coordinates
        x_min = percentage_x × width 
        x_min = percentage_x × width 
        x_max = percentage_(x+width) × width
        y_max = percentage_(y+height) × height
...
if __name__ == '__main__':
    ...
    model_name = 'PolypSAM_freeze_mask_decoder_vit_b_train_p0_50_test_p30_Kvasirbest_bs1_random_shot900_e100_Run1'
    n_images = 900
    perturb_h_len = 50
    perturb_l_len = 0
    ...
    parser.add_argument('--train_path', type=str, default='/content/drive/MyDrive/PP_SAM/dataset/TrainDataset/', help='path to train dataset')
    parser.add_argument('--test_path', type=str, default='/content/drive/MyDrive/PP_SAM/dataset/TestDataset/', help='path to testing Kvasir dataset')
    parser.add_argument('--train_save', type=str, default='./model_pth/' + model_name + '/')
    print("Section 2")
    # Ignore Jupyter arguments
    print("Section 2")

    opt, unknown = parser.parse_known_args(sys.argv[1:])
    ...
    # ---- build models ----
    model_type = 'vit_b'
    checkpoint = '/content/drive/MyDrive/PP_SAM/Model/sam_vit_b_01ec64.pth'#sam_vit_b_01ec64.pth' #sam_vit_l_0b3195.pth
    ...
    for k in img_idxs:
        ...
        x_perturbed_min = x_min - perturbation_factor × (x_max - x_min)
        y_perturbed_min = y_min - perturbation_factor × (y_max - y_min)
        x_perturbed_max = x_max + perturbation_factor × (x_max - x_min)
        y_perturbed_max = y_max + perturbation_factor × (y_max - y_min)
